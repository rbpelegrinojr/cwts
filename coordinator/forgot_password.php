              ...
              <div class="card card-plain mt-8">
                ...
                <form action="forgot_password.php" method="POST">
                  ...
                  if (isset($_REQUEST['btnCheckUname'])) {
                  	...
                  	$res = mysqli_fetch_assoc(mysqli_query($con, "SELECT * FROM coordinators_tbl WHERE email = '$email'"));

                  	if ($res['email'] == $email) {
                  		
                  		// $query = mysqli_query($con, "INSERT INTO ")
                  		$rand_code = mt_rand(100000,999999);
                  		$resCode = mysqli_fetch_assoc(mysqli_query($con, "SELECT * FROM coordinators_tbl WHERE code = '$rand_code'"));
                  		$member_name = $res['fname'].' '.$res['lname'];
                  		$sms_content = "Your Forgot Password Code is ".$rand_code;
                  		$date_sent = date('Y-m-d');
                  		if ($resCode['code'] != $rand_code) {
                  			
                  			$query = mysqli_query($con, "INSERT INTO sms_tbl (send_by, member_name, sms_content, contact_number, date_sent, sms_type, sms_status) VALUES ('admin', '$member_name', '$sms_content', '{$res['contact_number']}', '$date_sent', '1', '1')");

                  			if ($query) {
                  				$qCode = mysqli_query($con, "UPDATE coordinators_tbl SET code = '$rand_code' WHERE email = '{$res['email']}'");
