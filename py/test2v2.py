import tkinter as tk
from tkinter import ttk, messagebox
import serial
import serial.tools.list_ports
import time
import pymysql
from datetime import datetime, timedelta, date
import threading
import winsound

import os, sys

def get_com_ports():
    ports = serial.tools.list_ports.comports()
    arduino_ports = []

    for port in ports:
        if ("Arduino" in port.description) or ("CH340" in port.description) or ("USB-SERIAL CH340" in port.description):
            arduino_ports.append(port.device)

    return arduino_ports

arduino = None
def connect_arduino(port):
    global arduino
    try:
        arduino = serial.Serial(port, 9600, timeout=1)
        time.sleep(2)
        arduino.reset_input_buffer()
        update_log(f"Connected to {port}")
    except Exception as e:
        messagebox.showerror("Connection Error", f"Could not open {port}\n{e}")

db = pymysql.connect(
    host="localhost",
    user="root",
    password="",
    database="cwts"
)
cursor = db.cursor()

last_attendance = {}

root = tk.Tk()
root.title("Fingerprint Attendance")

log_display = tk.Text(root, height=10, width=50, state=tk.DISABLED)
log_display.grid(row=5, column=0, columnspan=3, pady=10)

def update_log(message):
    log_display.config(state=tk.NORMAL)
    log_display.insert(tk.END, message + "\n")
    log_display.config(state=tk.DISABLED)
    log_display.yview(tk.END)

# def play_ok_sound():
#     winsound.PlaySound("ok.wav", winsound.SND_FILENAME | winsound.SND_ASYNC)

# def play_error_sound():
#     winsound.PlaySound("error.wav", winsound.SND_FILENAME | winsound.SND_ASYNC)

import os, sys

def resource_path(relative_path):
    try:
        base_path = sys._MEIPASS
    except Exception:
        base_path = os.path.abspath(".")
    return os.path.join(base_path, relative_path)

def play_ok_sound():
    winsound.PlaySound(resource_path("ok.wav"), winsound.SND_FILENAME | winsound.SND_ASYNC)

def play_error_sound():
    winsound.PlaySound(resource_path("error.wav"), winsound.SND_FILENAME | winsound.SND_ASYNC)


def play_duplicate_sound():
    winsound.Beep(800, 200)
    winsound.Beep(600, 200)

def log_sms_to_db(finger_id, action_type):
    """
    Inserts a message into sms_tbl based on action_type ('login' or 'logout')
    """
    try:
        cursor.execute("""
            SELECT guardian_name, guardian_contact_number 
            FROM members_tbl 
            WHERE member_id = %s
        """, (finger_id,))
        guardian_info = cursor.fetchone()
        if not guardian_info:
            update_log(f"No guardian info found for ID {finger_id}")
            return
        
        guardian_name, guardian_contact = guardian_info

        if not guardian_contact:
            update_log(f"Guardian contact not found for {guardian_name}")
            return

        # Choose message based on login/logout
        if action_type == 'login':
            sms_content = f"Good day Mr./Mrs. {guardian_name}, this is to inform you that your son/daughter has logged in to the CWTS class."
        elif action_type == 'logout':
            sms_content = f"Good day Mr./Mrs. {guardian_name}, this is to inform you that your son/daughter has logged out to the CWTS class."
        else:
            return

        now = datetime.now()
        cursor.execute("""
            INSERT INTO sms_tbl (contact_number, sms_content, date_sent, sms_status)
            VALUES (%s, %s, %s, %s)
        """, (guardian_contact, sms_content, now, '1'))
        db.commit()
        update_log(f"SMS logged for {guardian_name} ({action_type})")

    except Exception as e:
        update_log(f"Error logging SMS: {e}")


def handle_attendance(finger_id):
    global cursor, db, port_var
    now = datetime.now()
    today = date.today()
    current_time = now.time()

    device_id = port_var.get()

    cursor.execute("""
        SELECT schedule_id, start_time, end_time, status, schedule_type
        FROM schedules_tbl
        WHERE schedule_date=%s AND device_id=%s AND status='active'
    """, (today, device_id))
    sched = cursor.fetchone()

    if not sched:
        update_log("No active schedule for today.")
        play_error_sound()
        return

    schedule_id, start_time, end_time, status, schedule_type = sched

    start_time = datetime.strptime(str(start_time), "%H:%M:%S").time()
    end_time = datetime.strptime(str(end_time), "%H:%M:%S").time()

    if not (start_time <= current_time <= end_time):
        update_log("⏰ Outside allowed schedule time.")
        play_error_sound()
        return

    if current_time > end_time:
        cursor.execute("UPDATE schedules_tbl SET status='inactive' WHERE schedule_id=%s", (schedule_id,))
        db.commit()
        update_log("📅 Schedule ended, marked inactive.")
        return

    if finger_id in last_attendance and now - last_attendance[finger_id] < timedelta(seconds=15):
        update_log(f"Duplicate scan")
        play_duplicate_sound()
        return
    last_attendance[finger_id] = now

    cursor.execute("SELECT fname, lname FROM members_tbl WHERE member_id=%s", (finger_id,))
    result = cursor.fetchone()
    if not result:
        update_log(f"ID {finger_id} not found in members_tbl.")
        play_error_sound()
        return

    full_name = f"{result[0]} {result[1]}"
    update_log(f"{full_name} scanned at {now.strftime('%Y-%m-%d %I:%M:%S %p')}")

    cursor.execute("""
        SELECT id, am_in, am_out, pm_in, pm_out 
        FROM attendance 
        WHERE user_id=%s AND schedule_id=%s AND DATE(timestamp)=%s
    """, (finger_id, schedule_id, today))
    record = cursor.fetchone()

    update_field = None
    if record:
        attendance_id, am_in, am_out, pm_in, pm_out = record

        if schedule_type == "morning":
            if not am_in:
                update_field = "am_in"
            elif not am_out:
                update_field = "am_out"

        elif schedule_type == "afternoon":
            if not pm_in:
                update_field = "pm_in"
            elif not pm_out:
                update_field = "pm_out"

        elif schedule_type == "both":
            noon_time = datetime.strptime("12:00", "%H:%M").time()
            if current_time < noon_time:
                if not am_in:
                    update_field = "am_in"
                elif not am_out:
                    update_field = "am_out"
            else:
                if not pm_in:
                    update_field = "pm_in"
                elif not pm_out:
                    update_field = "pm_out"

        if update_field:
            cursor.execute(f"UPDATE attendance SET {update_field}=%s WHERE id=%s", (now, attendance_id))
            db.commit()
            update_log(f"{update_field.replace('_', ' ').title()} recorded.")
            play_ok_sound()

            # Determine if login or logout
            if 'in' in update_field:
                log_sms_to_db(finger_id, 'login')
            elif 'out' in update_field:
                log_sms_to_db(finger_id, 'logout')

        else:
            update_log("All attendance slots already filled.")
            play_error_sound()

    else:
        if schedule_type == "morning":
            cursor.execute("""
                INSERT INTO attendance (user_id, schedule_id, timestamp, am_in)
                VALUES (%s,%s,%s,%s)
            """, (finger_id, schedule_id, now, now))
            db.commit()
            update_log("AM In recorded.")

        elif schedule_type == "afternoon":
            cursor.execute("""
                INSERT INTO attendance (user_id, schedule_id, timestamp, pm_in)
                VALUES (%s,%s,%s,%s)
            """, (finger_id, schedule_id, now, now))
            db.commit()
            update_log("PM In recorded.")

        elif schedule_type == "both":
            noon_time = datetime.strptime("12:00", "%H:%M").time()
            if current_time < noon_time:
                cursor.execute("""
                    INSERT INTO attendance (user_id, schedule_id, timestamp, am_in)
                    VALUES (%s,%s,%s,%s)
                """, (finger_id, schedule_id, now, now))
                db.commit()
                update_log("AM In recorded.")
            else:
                cursor.execute("""
                    INSERT INTO attendance (user_id, schedule_id, timestamp, pm_in)
                    VALUES (%s,%s,%s,%s)
                """, (finger_id, schedule_id, now, now))
                db.commit()
                update_log("PM In recorded.")

        play_ok_sound()

def listen_for_fingerprints():
    global arduino
    while True:
        if arduino and arduino.in_waiting:
            line = arduino.readline().decode().strip()
            if line.startswith("ID:"):
                try:
                    fid = int(line.split(":")[1])
                    if fid == 0:
                        update_log("Scan ERROR.")
                        play_error_sound()
                    else:
                        update_log(f"Detected ID: {fid}")
                        handle_attendance(fid)
                except ValueError:
                    update_log("Invalid fingerprint ID received.")

def start_scanning():
    if arduino:
        arduino.write(b's')
        update_log("Started scanning...")
    else:
        messagebox.showerror("Error", "No Arduino connected.")

def stop_scanning():
    if arduino:
        arduino.write(b'x')
        update_log("Stopped scanning.")

# ---------- COM Port Selector ----------
tk.Label(root, text="Select COM Port:").grid(row=0, column=0, padx=5, pady=5, sticky="w")

port_var = tk.StringVar()
ports_combo = ttk.Combobox(root, textvariable=port_var, values=get_com_ports(), state="readonly")
ports_combo.grid(row=0, column=1, padx=5, pady=5)

def connect_selected_port():
    port = port_var.get()
    if port:
        connect_arduino(port)
    else:
        messagebox.showwarning("Warning", "Please select a COM port.")

tk.Button(root, text="Connect", command=connect_selected_port).grid(row=0, column=2, padx=5, pady=5)

tk.Button(root, text='Start Scanning', command=start_scanning).grid(row=3, column=0, pady=4)
tk.Button(root, text='Stop Scanning', command=stop_scanning).grid(row=3, column=1, pady=4)

threading.Thread(target=listen_for_fingerprints, daemon=True).start()

root.mainloop()
