...
# Fingerprint attendance handling
def handle_attendance(finger_id):
    ...
    if finger_id not in last_attendance or now - last_attendance[finger_id] > timedelta(seconds=15):
        ...
        if record:
            ...
            # Handle PM IN / PM OUT logic
            if pm_in and not pm_out:
                # If PM In exists but PM Out doesn't, mark the next scan as PM Out
                update_field = "pm_out"
            elif not am_in:
                # If no AM In, mark this as AM In (morning scan)
                update_field = "am_in"
            elif not am_out and current_time < datetime.strptime("12:00", "%H:%M").time():
                # If it's morning time and AM Out is missing, mark as AM Out
                update_field = "am_out"
            elif not pm_in:
                # If PM In is missing, mark as PM In
                update_field = "pm_in"
            elif not pm_out:
                # If PM Out is missing, mark as PM Out
                update_field = "pm_out"
            ...
            # First scan today — create a new record and fill am_in or pm_in depending on the time
            if current_time >= datetime.strptime("12:00", "%H:%M").time():
                # If it's afternoon, store it in pm_in
