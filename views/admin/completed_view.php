<?php include 'header.php'; ?>
<link rel="stylesheet" type="text/css" href="../../include/DataTables/datatables.min.css">
<script type="text/javascript" src="../../include/DataTables/datatables.min.js"></script>
<!-- <div class="main_content_iner ">
<div class="container-fluid p-0 ">
<div class="row "> -->
	<style type="text/css">
		.main_content .main_content_iners {
		    min-height: 68vh;
		    transition: .5s;
		    position: relative;
		    /*background: #f3f4f3;*/
		    margin: 0;
		    /*z-index: 22;*/
		    border-radius: 0;
		    padding: 30px;
		}

		.main_content .main_content_iners.overly_inners::before {
		    position: absolute;
		    left: 0;
		    top: 0;
		    right: 0;
		    height: 160px;
		    z-index: -1;
		    background: #6C4632;
		    content: '';
		    border-radius: 0;
		    left: 0;
		}
	</style>
<div class="main_content_iners overly_inners ">
	<div class="container-fluid p-0 ">
		<div class="row">
	        <div class="col-12">
	            <div class="page_title_box d-flex align-items-center justify-content-between">
	                <div class="page_title_left">
	                    <h3 class="f_s_30 f_w_700 text_white">COMPLETED RESERVATIONS</h3>
	                    <ol class="breadcrumb page_bradcam mb-0">
	                    	<li class="breadcrumb-item"><a href="index">Dashboard </a></li>
	                    	<li class="breadcrumb-item active">Completed Reservations</li>
	                    </ol>
	                </div>
	            </div>
	        </div>
	    </div>

	    <div class="row ">

	    	<div class="col-lg-12">
			<div class="white_card card_height_100 mb_30">
				<div class="white_card_header">
					<div class="box_header m-0">
						<!-- <div class="main-title">
							
						</div> -->
					</div>
				</div>
				<div class="white_card_body">
					<div >
					<br>
					<?php
						
						$query = mysqli_query($con, "SELECT * FROM reservations_tbl WHERE reservation_status = '4' ORDER BY date_created ASC");
						if ($rows = mysqli_num_rows($query) > 0) {
								
							}else{
								?>
								<div class="alert alert-warning">
									<label>No Data Available.</label>
								</div>
								<?php
							}
						?>
					<div class="">
						<label><b>Total: <?php echo mysqli_num_rows($query); ?></b></label>
					</div>
					<br>
					
					<div >
						<table class="table table-bordered" id="myTable">
							<thead>
								<th class="text-center">#</th>
								<th class="text-center">User</th>
								<th class="text-center">Reservation Type</th>
								<th class="text-center">Remarks</th>
								<th class="text-center">Reservation Date & Time</th>
								<th class="text-center">Reservation Created</th>
								<th class="text-center">Date Completed</th>
								<!-- <th class="text-center">Requirements</th> -->
								<th class="text-center">Action</th>
							</thead>
							<tbody>
								<?php
								$i = 1;
								while ($row = mysqli_fetch_assoc($query)) {
									$resMem = mysqli_fetch_assoc(mysqli_query($con, "SELECT * FROM members_tbl WHERE member_id = '{$row['member_id']}'"));
									$resType = mysqli_fetch_assoc(mysqli_query($con, "SELECT * FROM reservation_type_tbl WHERE reservation_type_id = '{$row['reservation_type']}'"));
									?>
									<tr>
										<td class="text-center"><?php echo $i++; ?></td>
										<td class="text-center"><?php echo $resMem['fname'].' '.$resMem['lname']; ?></td>
										<td class="text-center"><?php echo $resType['reservation_type']; ?></td>
										<td class="text-center"><?php echo $row['reservation_description']; ?></td>
										<td class="text-center"><?php echo date('l F j, Y', strtotime($row['reservation_date'])).' @ '.date('h:i A', strtotime($row['reservation_time'])); ?></td>
										<td class="text-center"><?php echo $row['date_created']; ?></td>
										<td class="text-center"><?php echo $row['date_completed']; ?></td>
										<!-- <td class="text-center"><?php echo $row['requirements']; ?></td> -->
										<td class="text-center">
											<a href="#" title="Archive" onclick="archive(<?php echo $row['reservation_id']; ?>);"><span class="fa fa-archive"></span> Archive</a>
										</td>
									</tr>
									<?php
								}
								?>
							</tbody>
						</table>
					</div>
					</div>
				</div>
			</div>
		</div>

	    </div>
	</div>
</div>
<script type="text/javascript">
	function approveRes(r_id) {
		if (confirm('Approve Reservation?')) {
			window.location.href='../../controller/admin/process/save_data.php?btnApproveRes=1&r_id='+r_id+'&user_id=<?php echo $user_id; ?>';
		}
	}

	function declineRes(r_id) {
		if (confirm('Decline Reservation?')) {
			window.location.href='../../controller/admin/process/save_data.php?btnDeclineRes=1&r_id='+r_id+'&user_id=<?php echo $user_id; ?>';
		}
	}

	function archive(r_id) {
		if (confirm('Archive Record?')) {
			window.location.href='../../controller/admin/process/save_data.php?btnArchive=1&r_id='+r_id+'&user_id=<?php echo $user_id; ?>'
		}
	}
</script>
<script type="text/javascript">
	$(document).ready( function () {
	    $('#myTable').DataTable({
	    	"ordering": false
	    });
	});
</script>

<?php include 'footer.php'; ?>