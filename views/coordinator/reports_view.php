        ...
        <div class="col-lg-12">
            <div class="white_card card_height_100 mb_30">
                <div class="white_card_body">
                    ...
                    // Base query: join schedules to filter by coordinator
                    $sql = "SELECT a.*, s.schedule_type
                            FROM attendance a
                            LEFT JOIN schedules_tbl s ON a.schedule_id = s.schedule_id
                            WHERE s.coordinator_id = '{$resInfo['user_id']}'"; // only this coordinator's schedules

                    // Append month/year/week filters if any
                    if (!empty($where)) {
                    ...
                    if ($query && mysqli_num_rows($query) > 0) {
                        while ($row = mysqli_fetch_assoc($query)) {
                            $remarks = "Absent"; // default

                            if ($row['schedule_type'] == "morning") {
                                if (!empty($row['am_in']) && !empty($row['am_out'])) {
                                    $remarks = "Present";
                                } elseif (!empty($row['am_in']) || !empty($row['am_out'])) {
                                    $remarks = "Late";
                                }
                            } elseif ($row['schedule_type'] == "afternoon") {
                                if (!empty($row['pm_in']) && !empty($row['pm_out'])) {
                                    $remarks = "Present";
                                } elseif (!empty($row['pm_in']) || !empty($row['pm_out'])) {
                                    $remarks = "Late";
                                }
                                ...
                                <table class="table table-bordered">
                                    ...
                                    <tbody>
                                        ...
                                        foreach ($filtered_rows as $row) {
                                            ...
                                            // Prepare schedule type display
                                            $schedule_type_display = ucfirst($row['schedule_type']);

                                            // Display N/A for unused slots
                                        ...
                                        ?>
                                            <tr class="text-center">
                                                ...
                                                <td><?php echo $schedule_type_display; ?></td>
